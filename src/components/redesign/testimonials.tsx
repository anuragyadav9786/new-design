  ...
  return (
    <section id="testimonials" ref={ref} className="w-full bg-white py-[100px] sm:py-[140px]">
      <div className="mx-auto max-w-[1280px] px-[5vw]">
        <div
          className="flex flex-col items-center justify-between gap-6 transition-all duration-700 ease-[var(--tf-ease)] sm:flex-row sm:items-end"
          style={{
            opacity: isIntersecting ? 1 : 0,
            transform: isIntersecting ? "translateY(0)" : "translateY(16px)",
          }}
        >
          <h2 className="text-center text-[clamp(32px,4vw,56px)] font-bold leading-tight tracking-tight text-[var(--tf-navy)] sm:text-left">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-full border border-[var(--tf-border)] text-[var(--tf-text-secondary)] transition-colors duration-200 hover:border-[var(--tf-blue)] hover:text-[var(--tf-blue)]"
            >
        ...
        >
          {testimonials.map((t, i) => (
            <div
              key={t.name}
              className="w-[320px] shrink-0 snap-center rounded-[var(--tf-radius-lg)] border border-[var(--tf-border)] bg-[var(--tf-bg)] p-8 transition-all duration-700 ease-[var(--tf-ease)] sm:w-[380px]"
              style={{
                transitionDelay: isIntersecting ? `${150 + i * 100}ms` : "0ms",
                opacity: isIntersecting ? 1 : 0,
                transform: isIntersecting ? "translateY(0)" : "translateY(16px)",
              }}
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-[var(--tf-text)]">{t.name}</p>
